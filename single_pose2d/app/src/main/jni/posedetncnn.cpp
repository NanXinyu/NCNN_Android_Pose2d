...
JNIEXPORT jboolean JNICALL Java_com_tencent_ncnnbodypose_NcnnBodypose_loadModel(JNIEnv* env, jobject thiz, jobject assetManager, jint modelid, jint cpugpu)
{
    ...
    const int target_widths[] =
    {
        192,
        192,
    };

    const int target_heights[] =
    {
        256,
        256,
    };
...
//        {1 / 0.229f / 255.f, 1 / 0.224f / 255.f, 1 / 0.225f / 255.f},
    ...
    int target_width = target_widths[(int)modelid];
    int target_height = target_heights[(int)modelid];
    ...
    // reload
    {
        ...
        {
            if (!g_posedet)
                g_posedet = new PoseDet;
            g_posedet->load(mgr, modeltype, target_width, target_height, mean_vals[(int)modelid], norm_vals[(int)modelid], use_gpu);
        }
    }
